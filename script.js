...
document.addEventListener('DOMContentLoaded', () => {
  ...
  try { fetchNowPlayingArchive(); } catch(e){ console.error('fetchNowPlayingArchive error:', e); }
  try { shuffleIframesDaily(); } catch(e){ console.error('shuffleIframesDaily error:', e); }

  try { fetchLiveNow(); } catch(e) { console.error('fetchLiveNow error:', e); }
  try { fetchWeeklySchedule(); } catch(e) { console.error('fetchWeeklySchedule error:', e); }
  try { fetchNowPlayingArchive(); } catch(e) { console.error('fetchNowPlayingArchive error:', e); }  try { shuffleIframesDaily(); } catch(e) { console.error('shuffleIframesDaily error:', e); }
  ...
  if (popOutBtn) {
    popOutBtn.addEventListener('click', () => {
      ...
      pop.document.write(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>CCR Player</title></head><body><iframe src="\${src}" allow="autoplay" style="width:100%; height:100%; border:none;"></iframe></body></html>`);
      pop.document.close();
    });
  }
});
...
window.addEventListener("DOMContentLoaded", () => { try { shuffleIframesDaily(); } catch(e) { console.error("shuffleIframesDaily error:", e); } });
